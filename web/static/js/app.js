...
const FullscreenModal = {
  ...
  // Initialize the modal (call once on app init)
  init() {
    ...
    // Keyboard navigation is handled in App's global keydown handler
  },

  // Open the modal with images at a specific index
...
const ImageGallery = {
  ...
  render(images) {
    ...
    const thumbnailsHtml = this.images
      .map(
        (url, i) =>
    ...
    return `
            <div class="image-gallery" data-gallery>
                <div class="gallery-main">
                    <img src="${this.images[0]}" alt="Property image" class="gallery-main-img" style="cursor: pointer;" onerror="this.src='/static/img/no-image.png'">
                    ${
  ...
  init(container) {
    ...
    mainImg.addEventListener("click", () => {
      FullscreenModal.open(this.images, this.currentIndex);
    });
  },
...
const App = {
  ...
  initPropertySidebar() {
    ...
    document.addEventListener("keydown", (e) => {
      // Fullscreen modal takes priority
      if (FullscreenModal.handleKeydown(e)) {
        e.preventDefault();
        return;
      }
